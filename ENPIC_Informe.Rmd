...
paquetes <- c(
  "haven", "tidyverse", "nortest", "rstatix", "FSA", "ggpubr", "ggalluvial", 
  "ggvenn", "viridisLite", "patchwork", "gtsummary", "DiagrammeR", 
  "DiagrammeRsvg", "rsvg", "survival", "survminer", "car", "caret", "glmnet"
)
...
tabla_muestra <- caracterizacion_muestra(datos) %>%
  modify_caption("*Tabla 1.* Características de la muestra")
...
tabla_indicadores <- bind_rows(
  ...
  kable(
    caption = "*Tabla 2.* Resumen de indicadores",
...
```{r}
# Características de la muestra por aporte calórico
...
# Características de la muestra por aporte proteico
...
analisis_aporte <- function(datos, columna, valor_inf, valor_sup,
  ...
  return(
    kable(
      ...
      caption = paste(
        etiqueta, paste("Intervalo de referencia:", intervalo_ref),
        sep = "\n"
      ),
      align = "lcc"
    )
...
tabla_cal_estable <- analisis_aporte(
  ...
  etiqueta = "*Tabla 5.* Aporte calórico en fase estable (kcal/kg/día)"
)
...
tabla_prot_estable <- analisis_aporte(
  ...
  etiqueta = "*Tabla 6.* Aporte proteico en fase estable (g/kg/día)"
)
...
#' @param etiqueta Cadena de texto (opcional). Encabezamiento del título de la tabla.
#'
...
grupos_aporte <- function (datos, categoria, valor_ag, valor_es, 
  ...
  return(
    kable(
      df_tabla_cont,
      caption = paste(
        paste(etiqueta, "Evolución de los niveles de aporte", variable),
        paste("Test de McNemar:", p_text),
        sep = "\n"
      ),
      align = "lccc"
    )
...
tabla_evol_cal <- grupos_aporte(datos, "CALORIA", 15, 25, "*Tabla 7.*")
tabla_evol_prot <- grupos_aporte(datos, "PROT", 0.8, 1.3, "*Tabla 8.*")
...
tabla_cal_4 <- datos %>%
  ...
  analisis_aporte(CALORIA4, 25, 30, 
                  "*Tabla 9.* Aporte calórico día 4 (kcal/kg/día)")
...
tabla_prot_4 <- datos %>%
  ...
  analisis_aporte(PROT4, 1.3, 1.5, 
                  "*Tabla 10.* Aporte proteico día 4 (g/kg/día)")
...
tabla_dhanp_comp <- data.frame(
  ...
  kable(
    ...
    caption = "*Tabla 11.* Cumplimiento de criterios diagnósticos de DHANP",
    align = "lccccc"
  )
...
tabla_muestra_sup <- datos %>%
  ...
  modify_caption("*Tabla 12.* Características de la muestra por supervivencia")
...
datos_cox <- datos %>%
  ...
  # 4. Etiquetas de grupo diagnóstico
  mutate(
    STATUS3 = case_when(
      STATUS3 == "QUIRURGICO" ~ "Quirúrgico",
      STATUS3 == "MEDICO" ~ "Médico",
  ...
  # 5. Gestión de valores faltantes
...
tabla_cox_univ <- tabla_cox_univ %>%
  kable(
    ...
    caption = "*Tabla E5.* Análisis univariante de covariables",
    align = "lcccc"
  )
...
tabla_vif <- as.data.frame(tabla_vif) %>%
  kable(
    caption = "*Tabla E6.* Factores de inflación de la varianza (VIF)",
    align = "lccc"
  )
...
tabla_cox <- fit_cox %>%
  tbl_regression(
    ...
    label = list(
      NUTRIC_Score ~ "NUTRIC Score",
      VGSING2 ~ "Malnutrición (VGS)",
      Adecuacion_obj_prot ~ "Aporte proteico adecuado (≥80%)",
      EDAD ~ "Edad (años)",
      STATUS3 ~ "Grupo diagnóstico",
      SOFA1 ~ "SOFA (al ingreso)"
    )
  ...
  modify_caption("*Tabla 13.* Resultados de la regresión de Cox")
...
tabla_cox_zph <- as.data.frame(tabla_cox_zph$table) %>%
  kable(
    caption = "*Tabla E7.* Test de riesgos proporcionales",
    align = "lccc"
  )
...
#' progresion_aporte(df, "PROT")
#' }
comparacion_aporte <- function(datos, categoria, etiqueta = "") {
  # 1. Determinación de variables según la categoría
  caloria <- categoria == "CALORIA"
  variable <- if_else(caloria, "calórico", "proteico")
  unidades <- if_else(caloria, "kcal/kg/día", "g/kg/día")
  ...
  return(
    kable(
      tabla,
      caption = paste0(
        etiqueta, " Comparación de aporte ", variable, " (", unidades, ")"
      ),
      align = "crcrcr"
    )
  )
...
tabla_comp_cal <- comparacion_aporte(datos, "CALORIA", etiqueta = "*Tabla E1.*")
tabla_comp_prot <- comparacion_aporte(datos, "PROT", etiqueta = "*Tabla E2.*")
...
datos <- datos %>%
  ...
  filter(!is.na(cal_ok_es) & !is.na(prot_ok_es)) %>%
  
  
  mutate(
    Grupo_cal = case_when(
      cal_ok_ag & cal_ok_es ~ "FA + FE",
      cal_ok_ag & !cal_ok_es ~ "Solo FA",
      !cal_ok_ag & cal_ok_es ~ "Solo FE",
      !cal_ok_ag & !cal_ok_es ~ "Insuficiente"
    ),
    Grupo_cal = factor(Grupo_cal,
                        levels = c("FA + FE", 
                                   "Solo FA", 
                                   "Solo FE", 
                                   "Insuficiente")),
    
    Grupo_prot = case_when(
      prot_ok_ag & prot_ok_es ~ "FA + FE",
      prot_ok_ag & !prot_ok_es ~ "Solo FA",
      !prot_ok_ag & prot_ok_es ~ "Solo FE",
      !prot_ok_ag & !prot_ok_es ~ "Insuficiente"
    ),
    Grupo_prot = factor(Grupo_prot,
                        levels = c("FA + FE", 
                                   "Solo FA", 
                                   "Solo FE", 
                                   "Insuficiente"))
...
tabla_muestra_evol_cal <- caracterizacion_muestra(
  datos, comparar = "Grupo_cal", excluir = c("Kcal_14d", "gPROT_14d")
) %>%
  modify_caption(
    "*Tabla E3.* Características de la muestra por evolución de aporte calórico"
  )

tabla_muestra_evol_prot <- caracterizacion_muestra(
  datos, comparar = "Grupo_prot", excluir = c("Kcal_14d", "gPROT_14d")
) %>%
  modify_caption(
    "*Tabla E4.* Características de la muestra por evolución de aporte proteico"
  )
  
tabla_muestra_evol_cal
tabla_muestra_evol_prot

```

```{r, results='hide'}
# Correcciones post-hoc
categoria <- c("Grupo_cal", "Grupo_prot")
vars_dunn <- c("BMI", "SOFA1", "HORASIN", "TOTAL_DIAS_SN", 
               "DIASVM", "DIAS_UCI", "DIAS_HOSP")

for (cat in categoria) {
  for (var in vars_dunn) {
    formula_dunn <- as.formula(paste(var, "~", cat))
    test <- dunnTest(formula_dunn, data = datos, method = "bonferroni")
    table <- kable(test$res, digits = 3, caption = paste(var, cat))
    print(table)
  }
}

rm(categoria, vars_dunn, cat, var, formula_dunn, test, table)
