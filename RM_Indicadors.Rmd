...
progresion_aporte <- function(datos, categoria) {
  ...
  plot_out <- ggplot(datos, aes(x = Día, y = Mediana)) +
    # 1. Sombreado del RIQ
    geom_ribbon(aes(ymin = P25, ymax = P75), fill = viridis(5)[3], alpha = 0.2) +
    
    # 2. Línea de tendencia principal y puntos
    geom_line(color = viridis(5)[2], size = 1.2) +
    geom_point(color = viridis(5)[2], size = 3) +
    
    # 3. Líneas de objetivo
    annotate("segment", x = 1, xend = 3, y = fase_aguda, yend = fase_aguda,
             linetype = "dashed", color = viridis(5)[1], size = 0.8) +
    annotate("segment", x = 4, xend = 14, y = fase_estable, yend = fase_estable,
             linetype = "dashed", color = viridis(5)[4], size = 0.8) +
    ...
    annotate("text", x = 2.5, y = fase_aguda * 0.8, label = "Fase aguda", 
             color = viridis(5)[1], fontface = "bold", size = 3) +
    annotate("text", x = 9, y = fase_estable * 1.2, label = "Fase estable", 
             color = viridis(5)[4], fontface = "bold", size = 3)
...
umbrales_dhanp <- data.frame(
  Biomarcador = c("FALCALI7", "FALCALIA", "GAMGT7", "GAMGTA", "FILIRUB7", 
                  "FILIRUBA", "GOT7", "GOTA", "GPT7", "GPTA", "INR7", "INRA"),
  Corte = c(280, 280, 50, 50, 1.2, 1.2, 40, 40, 42, 42, 1.4, 1.4)
)

plot_dhanp_dist <- datos_dhanp %>%
  select(DHANP, FALCALI7, FALCALIA, GAMGT7, GAMGTA, FILIRUB7, FILIRUBA,
         GOT7, GOTA, GPT7, GPTA, INR7, INRA) %>%
  pivot_longer(
    cols = -DHANP,
    names_to = "Biomarcador",
    values_to = "Valor"
  )%>%
  filter(!is.na(Valor)) %>%
  
  ggplot(aes(x = as.factor(DHANP), y = Valor, fill = as.factor(DHANP))) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.3, size = 1) +
  
  geom_hline(data = umbrales_dhanp, aes(yintercept = Corte),
             linetype = "dashed", color = viridis(3)[2], size = 0.7) +
  
  facet_wrap(~ Biomarcador, scales = "free") +
  scale_y_log10() +
  scale_fill_viridis_d() +
  labs(
    title = "Distribución de biomarcadores vs. criterios diagnósticos",
    subtitle = "Eje y en escala logarítmica",
    x = "DHANP",
    y = "Valor",
    fill = "DHANP"
  ) +
