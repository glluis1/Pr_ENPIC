...
progresion_calorias <- datos %>%
  select(Peso_calculo, starts_with("CALORIA")) %>%
  pivot_longer(
    cols = starts_with("CALORIA"),
    names_to = "Día",
    names_prefix = "CALORIA",
    values_to = "kcal"
  ) %>%
  mutate(
    Dia = as.numeric(Día),
    kcal_kg = kcal / Peso_calculo
  ) %>%
  filter(!is.na(kcal_kg)) %>%
  
  group_by(Día) %>%
  summarise(
    Mediana = median(kcal_kg, na.rm = TRUE),
    P25 = quantile(kcal_kg, 0.25, na.rm = TRUE),
    P75 = quantile(kcal_kg, 0.75, na.rm = TRUE),
    .groups = "drop"
  )

progresion_calorias$Día <- as.numeric(as.character(progresion_calorias$Día))

# Gráfico de progresión temporal
ggplot(progresion_calorias, aes(x = Día, y = Mediana)) +
  # 1. Sombreado del error estándar
  geom_ribbon(aes(ymin = P25, ymax = P75),
              fill = "#21908CFF", alpha = 0.2) +
  
  # 2. Línea de tendencia principal y puntos
  geom_line(color = "#21908CFF", size = 1.2) +
  geom_point(aes(color = Mediana), size = 3) +
  
  # 3. Líneas de objetivo
  geom_segment(aes(x = 1, xend = 3, y = 12, yend = 12),
               linetype = "dashed", color = "orange", size = 0.8) +
  geom_segment(aes(x = 4, xend = 14, y = 20, yend = 20),
  ...
  scale_y_continuous(limits = c(
    0, max(progresion_calorias$P75, na.rm = TRUE) + 5
  )) +
  theme_minimal() +
  
  # 5. Texto y etiquetas
  labs(
    title = "Progresión del aporte calórico diario",
    subtitle = "Mediana de kcal/kg/día con rango intercuartílico",
    x = "Día",
    y = "Aporte (kcal/kg/día)",
    caption = "Las líneas discontinuas representan los objetivos para cada fase"
  ) +
  
  # 6. Anotaciones
  annotate("text", x = 2, y = 10, label = "Fase aguda", color = "orange",
           fontface = "bold", size = 3) +
  annotate("text", x = 9, y = 22, label = "Fase estable", color = "red",
           fontface = "bold", size = 3)
```
