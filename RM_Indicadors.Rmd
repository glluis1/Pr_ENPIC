...
# 1. Centrado de variables numéricas
datos_cox <- datos_cox %>%
  mutate(
    NUTRIC_Score = scale(NUTRIC_Score, center = TRUE, scale = FALSE),
    Adecuacion_obj_prot = scale(
      Adecuacion_obj_prot, center = TRUE, scale = FALSE
    ),
...
fit_train_full <- coxph(
  Surv(t, evento) ~ NUTRIC_Score + VGSING2 + Adecuacion_obj_prot + EDAD + 
    SEXO + ALCOHOL + AHTA + AEPOC + AIRC + AINMUNO + ANEOPLAS + STATUS3 +
    SOFA1 + TIPO_SN_Grupo + NEC_NA + TRR + INFVIA, 
  data = train_set
)
...
fit_cox <- coxph(
  Surv(t, evento) ~ 
    VGSING2 + Adecuacion_obj_prot + EDAD + STATUS3 + SOFA1 + INFVIA, 
  data = datos_cox
)
...
tabla_zph <- cox.zph(fit_cox)

plot_cox_zph <- ggcoxzph(tabla_zph)

# 9. Gráficos de residuos
