...
paquetes <- c("haven", "tidyverse", "nortest", "rstatix", "gtsummary", 
              "ggpubr", "ggalluvial", "ggvenn", "viridisLite", "patchwork",
...
El análisis de supervivencia se ha llevado a cabo utilizando el estimador de Kaplan-Meier para cada subgrupo. En el análisis multivariante subsiguiente se ha empleado un modelo escalonado hacia atrás (*backward stepwise*) de regresión de Cox para evaluar la mortalidad a los 28 días. Se han incluido variables en el modelo inicial con un valor $p < 0,2$ en el análisis univariante, así como covariables que se han considerado de relevancia clínica (e.g. edad, sexo, tipo de paciente o gravedad). Se ha empleado un modelo de aprendizaje automático basado en el criterio de información de Akaike (AIC, por sus siglas en inglés) para excluir variables del modelo final. Posteriormente al diagnóstico del modelo de regresión, se han eliminado variables con coeficiente de correlación de Spearman superior a 0,7 o que no cumplían la hipótesis de riesgos proporcionales.
...
fit_cox <- coxph(
  Surv(t, evento) ~ 
    VGSING2 + Adecuacion_obj_prot + EDAD + STATUS3 + SOFA1, 
  data = datos_cox
)
...
seleccion_cox <- function(datos, formula, vars_estudio, 
  ...
  fit_step <- coxph(formula, data = datos)
  bucle <- TRUE
  ...
  while(bucle && length(covariables) > 0) {
    ...
    res_step <- summary(fit_step)
    coef_mat <- res_step$coefficients
    
    print(fit_step)
    
    # 3.2. Filtraje de covariables
    coef_vars <- rep(names(fit_step$assign), lengths(fit_step$assign))
    
    filas <- coef_vars %in% covariables
    coef_step <- coef_mat[filas, , drop = FALSE]
    vars_step_map <- coef_vars[filas]
    ...
    hr_0 <- exp(coef(fit_step)[vars_all_step %in% vars_estudio])
    ...
    fit_new <- coxph(formula_step, data = datos)
    
    vars_all_new <- rep(names(fit_new$assign), lengths(fit_new$assign))
    hr_step <- exp(coef(fit_new)[vars_all_new %in% vars_estudio])
