...
progresion_aporte <- function(datos, categoria, comparar = FALSE) {
  ...
  datos <- datos %>%
    ...
    mutate(
      NUTRISCOR2 = factor(datos$NUTRISCOR2,
                          levels = c(0, 1),
...
# No s'aplica correcció per comparacions múltiples perquè en l'anàlisi exploratòria ja no hi ha cap p < 0,05
```
...
# Taules aportació: per a la indicació de NPC, taula dia 4
```
...
# Gràfic de distribució de temps d'inici de la NPC
