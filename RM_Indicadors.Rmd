...
caracterizacion_muestra <- function(datos, comparar = NULL, excluir = NULL) {
  ...
  variables <- c(
    ...
    "PBRONCOAS", "PISQUINT", "VM", "DIASVM", "NEC_NA", "TRR", "INFRESP",
    "INFVIA", "DIAS_UCI", "DIAS_HOSP", "MUERTO28"
  )
...
vars_cox <- c(
  "EDAD", "SEXO", "BMI", "ALCOHOL", "AHTA", "AEPOC", "AINMUNO", "ANEOPLAS", 
  "STATUS3", "APACHEII", "SOFA1", "NUTRIC_Score", "TIPO_SN_Grupo", "Kcal_14d", 
  "gPROT_14d", "VM", "NEC_NA", "TRR"
)
...
# 3. LASSO
x_train <- model.matrix(~ . -1, data = train_set[, vars_cox])
y_train <- Surv(train_set$t, train_set$evento)
...
x_test <- model.matrix(~ . -1, data = test_set[, vars_cox])

riesgo_lasso <- 
  predict(cv_lasso, newx = x_test, s = "lambda.min", type = "link")[, 1]

# 7. C-Index
eval_lasso <- coxph(Surv(t, evento) ~ riesgo_lasso, data = test_set)
c_index_lasso <- summary(eval_lasso)$concordance[1]

round(c_index_lasso, 3)

# 8. Modelo de Cox
coefs <- coef(cv_lasso, s = "lambda.min")
vars_select <- rownames(coefs)[which(coefs != 0)]

formula_cox <- as.formula(
  "Surv(t, evento) ~ EDAD + BMI + ALCOHOL + AHTA + AEPOC + AINMUNO + ANEOPLAS +
  STATUS3 + SOFA1 + NUTRIC_Score + TIPO_SN_Grupo + gPROT_14d + TRR"
)

fit_cox <- coxph(formula_cox, data = datos_cox)
...
tabla_cox <- fit_cox %>%
  ...
  bold_p(t = 0.05) %>%
  modify_header(label = "**Variable**", estimate = "**HR (IC 95%)**")
...
# 3. Ajuste saturado con el set de entrenamiento
formula <- as.formula(
  paste("Surv(t, evento) ~", paste(vars_cox, collapse = " + "))
)

fit_train_full <- coxph(formula, data = train_set)

# 4. Backwards stepwise selection
fit_train_final <- step(fit_train_full, direction = "backward", 
                        #trace = 0
                        )

# 5. Índice de riesgo para el test set
riesgo_test <- predict(fit_train_final, newdata = test_set, type = "lp")
...
fit_cox_step <- coxph(formula_final, data = datos_cox)

tabla_cox_step <- fit_cox_step %>%
  tbl_regression(
    exponentiate = TRUE
  ) %>%
  bold_p(t = 0.05) %>%
